...
用户可在 `openclaw.json` 中设置自定义端口（示例：`41623`），并确保浏览器扩展、Control UI 等处的 URL 与之一致。
...
2. **与防火墙策略一致**  
   「仅放行必要端口」时，若你只放行自己选定的端口（例如 41623），就不会依赖 18789 这个众所周知的默认值。
...
{
  "gateway": {
    "port": 41623,   // 本仓库使用；可改成你选定的端口
    "mode": "local",
    "bind": "loopback",
    "remote": {
      "url": "ws://127.0.0.1:41623"   // 必须与上面 port 一致
    }
    // ... 其余 gateway 配置不变
  }
}
...
- 启动时临时指定：  
  `openclaw gateway start --port 41623`  
...
| 位置 | 说明 |
|------|------|
| **`gateway.port`** | 改为新端口（如 41623）。 |
| **`gateway.remote.url`** | 例如 `ws://127.0.0.1:41623`，端口与 `gateway.port` 一致。 |
| **浏览器扩展 / Control UI** | 若扩展或书签里写死了 `http://127.0.0.1:18789`，需改成新端口（如 `http://127.0.0.1:41623`）。 |
...
- 建议使用 **1024–65535** 之间、不常见的高位端口（例如 41623、53417），避免与常见服务（22、80、443、3000 等）冲突。  
